...
//        os << obj.tempuser.id << ": " << obj.tempuser.name << "   import   " << obj.num << "  " << obj.ISBN << "  in  "
//           << obj.price1 << "  at  " << obj.time << endl;
//    } else {
//        os << obj.tempuser.id << ": " << obj.tempuser.name << "   buy   " << obj.num << "  " << obj.ISBN << "  in  "
//           << obj.price1 << "  at  " << obj.time << endl;
//    }
    return os;
}
...
class logemployee {
public:
    friend ostream &operator<<(ostream &os, const logemployee &obj);
...
void bookstore::report_employee(string name) {
    ...
    mystring caoni(buf);
    vector<sjtu::BPtree<mystring, int >::value_type> tempvec(cmd.the_blockchai_of_usermanager.find(caoni));
    if (tempvec.empty()) {
        cout << "GO FUCK  FW" << endl;
        return;
    }
    fa.close();
...
//    fa.write(reinterpret_cast<char *>(&templog), sizeof(templog));
    ...
    if (temp1 == "su") {
        ...
        string ISBN;
        for (i++; i < len && temp[i] != ' '; ++i) { ISBN += temp[i]; }
        if (i != len) error();
        else { select(ISBN); }
        ...
        if (i == len) {
            ...
            for (i++; i < len && temp[i] != ' '; ++i) { newpass += temp[i]; }
        ...
        if (i != len) error();
            ...
            if (ISBN_.empty() || quantity_.empty()) {
                error();
                return;
            }
            buy(ISBN_, quantity_);
...
bookstore::bookstore() :ISBN_BOOKSTORE_blocklist(ISBNBPT,ISBN_SEARCH),AUTHOR_BOOKSTORE(AUTHORBPT,AUTHOR_SEARCH),KEYWORD_BOOKSTORE(KETWORDBPT,KEYWORD_SEARCH),BOOKNAME_BOOKSTORE_blocklist(BOOKNAMEBPT,BOOK_NAME_SEARCH){
