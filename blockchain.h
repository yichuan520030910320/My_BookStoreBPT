...
public:
    ...
    char key[100];
public:
    element(const int &offset_ = -1, const string &key_ = " ");
...
//        f1.read(reinterpret_cast<char *>(&num),4);
            ...
            int location = upper_bound(tempblock.array, tempblock.array + tempblock.numofelment, index) -
                           tempblock.array;//todo
...
//cout<<" run out od the first while in deleteelement"<<endl;
        ...
        if (pos == tempblock.numofelment) {
            while (nxtblock(tempheadoffsey) != -1 && pos == tempblock.numofelment) {
                ...
                f2.read(reinterpret_cast<char *>(&tempblock), sizeof(block));
                location =
                        lower_bound(tempblock.array, tempblock.array + tempblock.numofelment, index) - tempblock.array;
                pos = location;
                while (pos != tempblock.numofelment) {
                    if ((strcmp(temp1, tempblock.array[pos].key) == 0) &&
                        tempblock.array[pos].offset == index.offset)
                        break;
                    pos++;
                }
            }
...
//cout<<temp<<"     "<<key_name<<"*******************"<<endl;
        ...
        int location = lower_bound(tempblock.array, tempblock.array + tempblock.numofelment, tempelement) -
                       tempblock.array;//todo
        ...
        } else//below may have bug
        {
            ...
            while (the_beginofblock_offset != -1) {
                ...
                f4.read(reinterpret_cast<char *>(&tempblock), sizeof(block));
                strcpy(tempelement.key, key_name.c_str());
                location = lower_bound(tempblock.array, tempblock.array + tempblock.numofelment, tempelement) -
                           tempblock.array;
    ...
    void debug() {
        ...
        cout << tempblock.numofelment << "blocklist debug the numofelement" << endl;
        for (int i = 0; i < tempblock.numofelment; ++i) {
            cout << tempblock.array[i].key << "___________*************____________" << tempblock.array[i].offset
                 << endl;
        }
        while (nxt != -1) {
            f3.seekg(nxt);
            f3.read(reinterpret_cast<char *>(&tempblock), sizeof(tempblock));
            for (int i = 0; i < tempblock.numofelment; ++i) {
                cout << tempblock.array[i].key << "&&&&& ---------**********    &&&&" << tempblock.array[i].offset
                     << endl;
            }
            nxt = nxtblock(nxt);
        }
